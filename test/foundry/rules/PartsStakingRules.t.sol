...
contract PartsStakingRulesTest is TestUtils {
    ...
    function setUp() public {
        ...
        partsRules.init(admin, absorberFactory, maxStakeableTotal, maxStakeablePerUser, powerFactor);
    }
    ...
    function test_getUserPower(
        ...
        assertEq(partsRules.getUserPower(_user, _nft, _tokenId, _amount), 0);
    }
    ...
    function test_getAbsorberPower() public {
        ...
        vm.mockCall(nftHandler, abi.encodeCall(INftHandler.absorber, ()), abi.encode(absorber));
        vm.mockCall(address(absorber), abi.encodeCall(IAbsorber.callUpdateRewards, ()), abi.encode(true));

        vm.mockCall(absorber, abi.encodeCall(IAbsorber.isUserExceedingDepositCap, (user)), abi.encode(false));

        vm.prank(absorberFactory);
        partsRules.setNftHandler(nftHandler);

        uint256[4][8] memory testData = [
            // staked, maxStakeableTotal, powerFactor, absorberPower
            [uint256(1), 10, 1e18, 119e16],
            [uint256(2), 10, 1e18, 136e16],
            // vary maxStakeableTotal and staked
            [uint256(1), 800, 1e18, 10024984375e8],
            // absorberPower = 1.0024984375
            [uint256(2), 800, 1e18, 100499375e10],
            // absorberPower = 1.00499375
            [uint256(20), 800, 1e18, 1049375e12],
            // absorberPower = 1.049375
            [uint256(40), 800, 1e18, 10975e14],
            // absorberPower = 1.0975
        ...
        for (uint256 i = 0; i < testData.length; i++) {
            ...
            vm.prank(admin);
            partsRules.setPowerFactor(power);

            assertEq(partsRules.getAbsorberPower(), absorberPower);
    ...
    function test_processUnstake(
        ...
        vm.mockCall(absorber, abi.encodeCall(IAbsorber.isUserExceedingDepositCap, (_user)), abi.encode(true));
        ...
        vm.mockCall(absorber, abi.encodeCall(IAbsorber.isUserExceedingDepositCap, (_user)), abi.encode(false));
    ...
    function test_setNftHandler(address _nftHandler) public {
        ...
        assertEq(partsRules.hasRole(partsRules.SR_ABSORBER_FACTORY(), absorberFactory), true);

        bytes memory errorMsg = TestUtils.getAccessControlErrorMsg(address(this), partsRules.SR_ABSORBER_FACTORY());
        vm.expectRevert(errorMsg);
        ...
        assertEq(partsRules.hasRole(partsRules.SR_ABSORBER_FACTORY(), absorberFactory), false);
    }
    ...
    function test_setPowerFactor(uint256 _powerFactor) public {
        mockCallUpdateRewards();

        vm.assume(_powerFactor > 1e17);

        bytes memory errorMsg = TestUtils.getAccessControlErrorMsg(address(this), partsRules.SR_ADMIN());
        vm.expectRevert(errorMsg);
        partsRules.setPowerFactor(_powerFactor);

        vm.prank(admin);
        vm.expectEmit(true, true, true, true);
        emit PowerFactor(_powerFactor);
        partsRules.setPowerFactor(_powerFactor);
        assertEq(partsRules.powerFactor(), _powerFactor);
    }
}

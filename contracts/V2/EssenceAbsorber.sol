...
contract EssenceAbsorber is Initializable, AccessControlEnumerableUpgradeable, ERC1155HolderUpgradeable {
    ...
    function stakeArtifact(uint256 _tokenId) external virtual updateRewards {
        ...
        uint256 power = getNftPower(artifact, _tokenId);
    ...
    function getNftPower(address _nft, uint256 _tokenId) public view virtual returns (uint256) {
        if (_nft == artifact) {
            try IArtifactMetadataStore(artifactMetadataStore).metadataForArtifact(_tokenId) returns (
                IArtifactMetadataStore.ArtifactMetadata memory metadata
            ) {
    ...
    function setTreasure(address _treasure) external virtual onlyRole(FACTION_ABSORBER_ADMIN_ROLE) {
        treasure = _treasure;
    }

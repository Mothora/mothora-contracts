...
abstract contract StakingRulesBase is IStakingRules, AccessControlEnumerableUpgradeable {
    ...
    /// @dev temporary role assigned to absorber factory to setup nftHandler after it's deployed
    ///      (solves circular dependency)
    bytes32 public constant SR_ABSORBER_FACTORY = keccak256("SR_ABSORBER_FACTORY");
    ...
    function _initStakingRulesBase(address _admin, address _absorberFactory) internal onlyInitializing {
        __AccessControlEnumerable_init();

        _setRoleAdmin(SR_ADMIN, SR_ADMIN);
        _setRoleAdmin(SR_NFT_HANDLER, SR_ADMIN);
        _setRoleAdmin(SR_ABSORBER_FACTORY, SR_ADMIN);

        _grantRole(SR_ADMIN, _admin);
        // SR_NFT_HANDLER must be a contract that implements absorber() getter
        _grantRole(SR_ABSORBER_FACTORY, _absorberFactory);
    }
    ...
    function processStake(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) external override onlyRole(SR_NFT_HANDLER) {
    ...
    /// @inheritdoc IStakingRules
    function setNftHandler(address _nftHandler) external onlyRole(SR_ABSORBER_FACTORY) {
        nftHandler = INftHandler(_nftHandler);

        _grantRole(SR_NFT_HANDLER, _nftHandler);
        _revokeRole(SR_ABSORBER_FACTORY, msg.sender);
    }

...
/// @dev - one NFT handler per absorber
    ...
    ERC1155HolderUpgradeable
{
    ...
    function stakeNft(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) public nonReentrant validateInput(_nft, _amount) processStake(msg.sender, _nft, _tokenId, _amount) {
    ...
    function unstakeNft(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) public nonReentrant validateInput(_nft, _amount) processUnstake(msg.sender, _nft, _tokenId, _amount) {
        Interfaces supportedInterface = getSupportedInterface(_nft, _tokenId);
    ...
    function replaceExtractor(
        address _nft,
        uint256 _tokenId,
        uint256 _amount,
        uint256 _replacedSpotId
    ) public validateInput(_nft, _amount) {
        ...
        (address user, uint256 replacedTokenId, uint256 replacedAmount) = stakingRules.canReplace(
            msg.sender,
            _nft,
            _tokenId,
            _amount,
    ...
    function _setNftConfig(
        ...
        if (newStakingRules != oldStakingRules) {
            ...
            } else if (newStakingRules == address(0)) {
                // remove
